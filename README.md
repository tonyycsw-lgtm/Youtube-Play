...
- 目前**只做新增**，沒有編輯／刪除；誤加或重複需到 Cloudflare Dashboard → KV 刪除 `program:{videoId}` 與 `vocab:{videoId}`。
...
- 詞匯內容**一律由老師提供的 JSON 決定**（每個片段有 `start`／`end` 與 `words`）。
- 每個詞顯示為 `原形 (原型)`（例如 `thought (think)`）；點詞會把中文釘在上方放大顯示並**暫停影片**。
...
- 無登入/班級管理；所有學生共用同一清單。
- 首頁「新增影片」不設保護、也尚無編輯/刪除；任何取得網址的人都能新增（資料存 KV）。
